...
static void handle_add_drm_card(struct wl_listener *listener, void *data) {
	...
	wlr_log(WLR_DEBUG, "Received session_add_drm_card signal: %s", event->path);
	...
	wlr_log(WLR_DEBUG, "Opened %s as DRM device, fd = %d", event->path, wlr_dev->fd);

	struct wlr_backend *child_drm = wlr_drm_backend_create(
		backend_monitor->session->display,
		backend_monitor->session,
		wlr_dev,
		backend_monitor->primary_drm);
	if (!child_drm) {
	...
	if (!wlr_backend_start(child_drm)) {
		wlr_log(WLR_ERROR, "Failed to start new child drm backend, destroying it.");
		wlr_backend_destroy(child_drm);
	}
}
...
static void handle_session_destroy(struct wl_listener *listener, void *data) {
	...
	destroy_drm_backend_monitor(backend_monitor);
}
