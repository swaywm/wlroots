...
/**
 * Will log all messages less than or equal to `verbosity`. If `callback` is
 * NULL, wlr will use its default logger.
 */
...
void _wlr_log(enum wlr_log_importance verbosity, const char *format, ...)
	_WLR_ATTRIB_PRINTF(2, 3);
void _wlr_vlog(enum wlr_log_importance verbosity, const char *format,
	va_list args) _WLR_ATTRIB_PRINTF(2, 0);
