...
static void manager_handle_get_input_timestamps(struct wl_client *client,
	...
	switch (input_timestamps_type) {
	case WLR_INPUT_TIMESTAMPS_KEYBOARD:
		wlr_signal_emit_safe(&manager->events.new_keyboard_timestamps,
			input_timestamps);
		break;
	case WLR_INPUT_TIMESTAMPS_POINTER:
		wlr_signal_emit_safe(&manager->events.new_pointer_timestamps,
			input_timestamps);
		break;
	case WLR_INPUT_TIMESTAMPS_TOUCH:
		wlr_signal_emit_safe(&manager->events.new_touch_timestamps,
			input_timestamps);
		break;
	}
...
struct wlr_input_timestamps_manager_v1 *wlr_input_timestamps_manager_v1_create(struct wl_display *display) {
	...
	wl_signal_init(&manager->events.new_keyboard_timestamps);
	wl_signal_init(&manager->events.new_pointer_timestamps);
	wl_signal_init(&manager->events.new_touch_timestamps);
